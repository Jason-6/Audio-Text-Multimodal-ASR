...
def collate(sample_list, tokenizer, config):
    ...
    return ((a_inputs, a_attention_mask),
            batch_label, batch_name, label_len, audio_lens)
...
def run(args, config, train_data, valid_data):
    ...
    while step < max_step:
        ...
        ASRModel.train()
        for acoustic_inputs, label_inputs, _, label_len, audio_lens in train_loader:
            ...
            total_loss = 0
            real_len, att_output, att_align, dec_state= ASRModel(
            ...
            if (step == 1) or (step % valid_step == 0):
                ...
                if avg_cer < min_cer:
                    min_cer = avg_cer
                    # 保存当前最佳模型参数到文件中
                    ckppdir = '/vol/nfs/mgl/GST/3_31_ctaL/CTAL-main/code/LK_data_cut_off/LK_data_cutoff_result'
...
if __name__ == '__main__':
    ...
    train_data_table = '/vol/nfs/mgl/GST/3_31_ctaL/CTAL-main/dataset/20h_parallel_forfinntune.csv'
