...
def run(args, config, train_data, valid_data):
    ...
    att_loss = None
    step = 0
    valid_step = 7506 #one epoch do one valid
    max_step = 375300 #40 epochs
    # 初始化最小CER值为一个很大的数
    min_cer = 1000.0
    min_ser = 1000.0
    # 初始化最小CER值对应的模型参数
    ...
    valid_log_file = '/vol/nfs/mgl/GST/3_31_ctaL/CTAL-main/Ablation_Study_AISHELL_Pretrained/without_CMAM/decode_log'
        ...
        for acoustic_inputs, semantic_inputs, label_inputs, _, label_len, audio_lens, text_lens in train_loader:
            ...
            if (step == 1) or (step % valid_step == 0):
                ...
                if avg_cer < min_cer:
                    min_cer = avg_cer
                    # 保存当前最佳模型参数到文件中
                    ckppdir = '/vol/nfs/mgl/GST/3_31_ctaL/CTAL-main/Ablation_Study_AISHELL_Pretrained/without_CMAM/result'
                    ckpt_path = os.path.join(ckppdir, "best_att.pth")
                    full_dict = {
                        "model": ASRModel.state_dict(),
                        "semantic_model": ASRModel.encoder.semantic_model.state_dict(),
                        "acoustic_model": ASRModel.encoder.acoustic_model.state_dict(),
                       # "decoder":ASRModel.decoder.state_dict(),
                        #"attention":ASRModel.attention.state_dict(),  
...
if __name__ == '__main__':
    ...
    report_result = []
    #
    train_data_root = '/vol/nfs/mgl/GST/3_31_ctaL/CTAL-main/dataset/train'
    # gst change 2.27(将train_data,valid_data,test_data传入):
    train_data_table = '/vol/nfs/mgl/GST/3_31_ctaL/CTAL-main/dataset/train_aishell_downstream.csv'
    ...
    # valid_data:
    valid_data_root = '/vol/nfs/mgl/GST/3_31_ctaL/CTAL-main/dataset/dev'
    valid_data_table = '/vol/nfs/mgl/GST/3_31_ctaL/CTAL-main/dataset/dev_aishell_downstream_change.csv'
