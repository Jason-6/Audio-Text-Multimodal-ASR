...
'''
combine with plot_attention_weight
'''
def plot_attention_weights(aw, tokens=[],step=0, spectrogram=None, factor=4,
    ...
    if save_path is not None:
        file_name = os.path.join(save_path,'attention_{}.png'.format(step))
...
"""June 2023 change(X)"""
def _save_canvas(data, meta=None):
    # fig, ax = plt.subplots(figsize=(len(data[1]), len(data[0])))
    max_size = max(len(data[1]), len(data[0]))  # 计算最大边长
    ...
    original_yticklabels = ax.get_yticklabels()

    # 进行等比例缩放
    fig.set_size_inches(max_size, max_size)

    # 根据缩放比例调整刻度标签的位置和数值
    scaled_yticks = original_yticks * max_size / len(data[0])

    # 将调整后的刻度标签应用到图像上
    ax.set_yticks(scaled_yticks)
    ax.set_yticklabels(original_yticklabels)
    # ax.imshow(data, aspect="auto", origin="lower")  #李宏毅老师原来的版本
    ax.imshow(data, aspect="auto", origin="lower")#横轴代表的是文本每个字的位置，纵轴表示的是语音时间步，使用data.T进行调换
    font_path = "/home/geshuting/SimSun.ttf"
    font_prop = fm.FontProperties(fname=font_path)
    ax.set_yticks(range(len(meta)))
    ax.set_yticklabels(meta,fontproperties=font_prop)
    # else:
    #     ax.bar(meta[0], data[0], tick_label=meta[1], fc=(0, 0, 1, 0.5))
    #     ax.bar(meta[0], data[1], tick_label=meta[1], fc=(1, 0, 0, 0.5))
    fig.canvas.draw()
...
def feat_to_fig(feat,meta):
    # feat TxD tensor
    data = _save_canvas(feat.numpy(),meta)
...
# def collate_train(sample_list, tokenizer, config):
#     batch_audio = [x['audio_input'] for x in sample_list]
#     audio_lens = [x['audio_input'].shape[0] for x in sample_list]  # 获取音频序列长度
#     pad_batch_audio = pad_sequence(batch_audio, batch_first=True)
#     # for x in sample_list:
...
def run(args, config, train_data, valid_data):
    ...
    att_loss = None
    step = 0
    valid_step = 1096 #one epoch do one valid
    max_step = 98640  #30 epochs
    # 初始化最小CER值为一个很大的数
    min_cer = 1000.0
    # 初始化最小CER值对应的模型参数
    ...
    valid_log_file = '/home/geshuting/Code/CTAL-main/CTAL-main/6_19_test_valid_log.txt'
        ...
        for acoustic_inputs, semantic_inputs, label_inputs, _, label_len, audio_lens, text_lens in train_loader:
            ...
            if (step == 1) or (step % valid_step == 0):
                ...
                for i, (
                    ...
                    # log_dict_valcer = {'tr_att': cal_er(tokenizer, att_output, label_inputs)}
                    ...
                    #         true_label = re.sub(r'\[CLS\]|\[SEP\]|\[PAD\]', '', true_text).strip()
                    #         log_dict_label = list(true_label)
                    #
                    #         # log_name_align = 'att_align{}'.format(i)
                    #         # log_dict_align = feat_to_fig(
                    #         #     att_align[j, 0, :, :].cpu().detach(),log_dict_label)
                    #         # img, form = log_dict_align
                    #         # if len(log_dict_align) > 0:
                    #         #     log.add_image(log_name_align, img, global_step=step, dataformats=form)
                    #         data = att_align[j,0,:,:].cpu().detach()
                    #         aw =  np.expand_dims(data,axis=0)
                    #         save_path = '/home/geshuting/attention_FIG'
                    #         plot_attention_weights(aw=aw, tokens=log_dict_label, step=step, save_path=save_path)
                ...
                if avg_cer < min_cer:
                    min_cer = avg_cer
                    # 保存当前最佳模型参数到文件中
                    ckppdir = '/home/geshuting/Code/CTAL-main/CTAL-main/6_19_test/with_LAS_output'
                    ...
                    verbose("Saved checkpoint (step = {}, CER = {:.2f}) and status @ {}".format(human_format(step),
                                                                                                min_cer, ckpt_path))


                # ckppdir = '/home/geshuting/Code/Low-Resource-Multimodal-Pre-training-master/with_LAS_output/'
                # ckpt_path = os.path.join(ckppdir, "latest.pth")
                # full_dict = {
                #     "model": ASRModel.state_dict(),
                #     "optimizer": optimizer.state_dict(),
                #     "global_step": step,
...
if __name__ == '__main__':
    ...
    report_result = []
    #
    train_data_root = '/home/geshuting/dataset/100%_data/20h_parallel'
    # gst change 2.27(将train_data,valid_data,test_data传入):
    train_data_table = '/home/geshuting/Code/CTAL-main/CTAL-main/dataset/20h_parallel_forfinntune.csv'
    ...
    # valid_data:
    valid_data_root = '/home/geshuting/dataset/downstream_val'
    valid_data_table = '/home/geshuting/Code/CTAL-main/CTAL-main/dataset/downstream_val.csv'
    valid_tables = [pd.read_csv(valid_data_table)]
    valid_tables = pd.concat(valid_tables, ignore_index=True).sort_values(by=['length'], ascending=False)
