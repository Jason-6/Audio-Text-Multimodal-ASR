...
class BaseDataset(Dataset):
    def __init__(self, file_path, sets, bucket_size, max_timestep=0, drop=False):
        ...
        # drop the results with some blank txt
        # self.table = self.table.dropna(axis=0)
        self.table = self.table.dropna(axis=1)
...
def load_semantic_data(txt_path, txt_root=None, tokenizer=None):
    txt_content = ' '.join(
        [x.strip('\n').split(',')[0] for x in open(os.path.join(txt_root, txt_path), 'r').readlines()])
    txt_content = tokenizer(txt_content)
    return txt_content
...
class MultiModalDataset(BaseDataset):
    def __init__(self, file_path, sets, bucket_size, max_timestep=0, drop=False,
                 acoustic_config=None, semantic_config=None, tokenizer=None, mask_proportion=0.15):
    ...
    def process_x_pad_batch(self, x_a_pad_batch, x_t_pad_batch, audio_lens, true_label):
        ...
        #self.tokenizer.pad函数只接受字典（dict）作为输入
        x_t_pad_batch = self.tokenizer.pad(x_t_pad_batch, return_tensors="pt")
        true_label = self.tokenizer.pad(true_label, return_tensors="pt")
        label_inputs = true_label['input_ids']
        # true_label = pad_sequence([torch.tensor(l) for l in true_label], batch_first=True, padding_value=0)
        s_inputs, s_labels = mask_tokens(x_t_pad_batch['input_ids'],mlm_probability=self.mask_proportion,
                                         tokenizer=self.tokenizer,tail_masking=False)
        ...
        label_inputs = label_inputs[valid_batchid]


        return batch_is_valid, audio_lens, (a_inputs, a_mask_labels, a_attention_mask, a_labels), (s_inputs,s_attention_mask,s_labels)

    def __getitem__(self, index):
        acoustic_batch = [load_acoustic_data(x_file, self.root) for x_file in self.X_a[index]]
        #gst 5.10 change:
        audio_lens = [x.shape[0] for x in acoustic_batch]
        ...
        true_label['input_ids'] = x_t_pad_batch['input_ids']
        true_label['attention_mask'] = x_t_pad_batch['attention_mask']
        return self.process_x_pad_batch(x_a_pad_batch, x_t_pad_batch, audio_lens, true_label)
...
class MultiModalDataset_Chinese(BaseDataset):
    ...
    def process_x_pad_batch(self, x_a_pad_batch, x_t_pad_batch, audio_lens, true_label):
        ...
        true_label = self.tokenizer.pad(true_label, return_tensors="pt")
        label_inputs = true_label['input_ids']
        s_inputs, s_labels = mask_tokens(x_t_pad_batch['input_ids'])
        s_attention_mask = x_t_pad_batch['attention_mask']
        s_valid_batchid = torch.nonzero(torch.sum(s_labels!=-100,dim=1),as_tuple=False).view(-1)
        #---------- process the valid batch id ----------#
        a_valid = torch.zeros(a_labels.size(0))
        ...
        s_labels = s_labels[valid_batchid]
        label_inputs = label_inputs[valid_batchid]
        return batch_is_valid, audio_lens, (a_inputs, a_mask_labels, a_attention_mask, a_labels), (s_inputs,s_attention_mask,s_labels,label_inputs)
